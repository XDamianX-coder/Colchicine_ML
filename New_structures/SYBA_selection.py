...
if __name__ == "__main__":
    ...
    try:
        #A549
        model = joblib.load('../Activity/Random_forest/random_forest_model_2_estimators_A549.joblib')
        ...
        #BALB/3T3
        model = joblib.load('../Activity/Random_forest/random_forest_model_2_estimators_BALB_3T3.joblib')
        mob = model.predict(molecular_descriptors[list(model.feature_names_in_)])
        converted = pred_model.inverse_transform(mob)
        df_gen_fin['BALB/3T3 [nM]'] = converted

        #LoVo
        model = joblib.load('../Activity/Random_forest/random_forest_model_3_estimators_LoVo.joblib')
        mob = model.predict(molecular_descriptors[list(model.feature_names_in_)])
        converted = pred_model.inverse_transform(mob)
        df_gen_fin['LoVo [nM]'] = converted

        #LoVo/DX
        model = joblib.load('../Activity/Random_forest/random_forest_model_8_estimators_LoVo_DX.joblib')
        mob = model.predict(molecular_descriptors[list(model.feature_names_in_)])
        converted = pred_model.inverse_transform(mob)
